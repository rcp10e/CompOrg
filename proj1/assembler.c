...
main(int argc, char *argv[])
{
	...
	for(i = 0; i < lineCount; i++)
	{
		...
		{
			if(!strcmp(labelArray[i].labelName, labelArray[j].labelName) && (i != j))
			{
				if(strcmp(labelArray[j].labelName, "       ") && strcmp(labelArray[j].labelName, "")
					&& strcmp(labelArray[j].labelName, "\t") && strcmp(labelArray[j].labelName, " ")
					&& strcmp(labelArray[j].labelName, "\0"))
				{
					printf("Error: duplicate labels at %d and %d\n", i, j);
					exit(1);
				}
			}
		}
	...
	for(i = 0; readAndParse(inFilePtr, label, opcode, arg0, arg1, arg2); i++)
	{
		labelFlag = 0;
		if(!isNumber(arg0))
		{
			...
			{	
				printf("error: undefined label in arg0 at address %d\n", i);
				exit(1);
			}
		}
		labelFlag = 0;
		if(!isNumber(arg1))
		{
			...
			if(labelFlag == lineCount)
			{
				printf("error: undefined label in arg1 at address %d\n", i);
				exit(1);
			}
		}
		...
		if(!isNumber(arg2))
		{
			...
			if(labelFlag == lineCount)
			{
				printf("error: undefined label in arg2 at address %d\n", i);
				exit(1);
			}
		}
	}
